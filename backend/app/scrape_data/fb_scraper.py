...
from ai_analysis import DEFAULT_GEMINI_MODEL, analyze_records, configure_gemini
...
def parse_args():
    ...
    parser.add_argument("--analyze", dest="analyze", action="store_true", help="Tự động gọi AI sau khi cào xong")
    parser.add_argument("--no-analyze", dest="analyze", action="store_false", help="Tắt bước phân tích AI tự động")
    parser.set_defaults(analyze=True)
    parser.add_argument("--analysis-model", default=DEFAULT_GEMINI_MODEL, help="Model Gemini dùng cho bước phân tích")
    parser.add_argument("--api-key", help="Gemini API key (nếu không dùng .env)")
    parser.add_argument("--keep-raw", action="store_true", help="Giữ lại file JSON thô sau khi phân tích")
...
async def main():
    ...
    if args.analyze:
        ...
        console.print("\n[cyan]→ Đang phân tích JSON bằng Gemini...[/cyan]")
        configure_gemini(api_key)
        model = genai.GenerativeModel(args.analysis_model)
        analyzed = await analyze_records([p.to_dict() for p in all_posts], model)
        Path(f"analyzed_{base}.json").write_text(json.dumps(analyzed, ensure_ascii=False, indent=2), encoding="utf-8")
        console.print(f"[green]✓ Output cuối:[/green] analyzed_{base}.json")
